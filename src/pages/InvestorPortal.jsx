...
import { InvestorUpdateDB, SignNowRequestDB } from '@/api/entities';
...
// ─── Request Investment Documents Modal ──────────────────────────────────
function RequestDocumentsModal({ portalUser, onClose, onSuccess }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (templates.length === 0) throw new Error('No document templates are configured. Please contact the administrator.');
      ...
      setErrorMsg(e.message || 'An error occurred. Please contact support.');
      setStep('error');
    }
  };
      ...
      <div style={{ background: '#0d1b2a', border: '1px solid rgba(184,147,58,0.35)', borderRadius: '2px', padding: '44px', maxWidth: '520px', width: '100%', boxShadow: '0 40px 100px rgba(0,0,0,0.8)' }}>
        {step === 'confirm' && (<>
          ...
          <div style={{ background: 'rgba(184,147,58,0.08)', border: '1px solid rgba(184,147,58,0.2)', borderRadius: '2px', padding: '16px', marginBottom: '28px' }}>
            ...
            <div style={{ color: '#4a5568', fontSize: '11px', marginTop: '4px' }}>This is the email address on file for your account.</div>
          </div>
        ...
        {step === 'sending' && (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            <div style={{ width: '44px', height: '44px', border: '3px solid rgba(184,147,58,0.2)', borderTop: `3px solid ${GOLD}`, borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 20px' }} />
            <p style={{ color: '#8a9ab8', fontSize: '14px' }}>Sending documents via SignNow…</p>
        ...
        {step === 'done' && (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <h3 style={{ color: '#4ade80', margin: '0 0 12px', fontFamily: 'Georgia, serif', fontWeight: 'normal' }}>Documents Sent!</h3>
            <p style={{ color: '#8a9ab8', fontSize: '13px', lineHeight: 1.7, margin: '0 auto 16px', maxWidth: '360px' }}>
              Documents have been sent to <strong style={{ color: GOLD }}>{portalUser.email}</strong> via SignNow. Check your inbox and complete the signature process.
            </p>
            <p style={{ color: '#4a5568', fontSize: '12px', marginBottom: '28px' }}>Track status in <strong style={{ color: '#8a9ab8' }}>Account → Documents</strong>.</p>
            <button onClick={() => { onSuccess && onSuccess(); onClose(); }} style={{ background: 'linear-gradient(135deg,#b8933a,#d4aa50)', color: DARK, border: 'none', borderRadius: '2px', padding: '12px 36px', cursor: 'pointer', fontWeight: '700', fontSize: '12px', letterSpacing: '2px', textTransform: 'uppercase' }}>Close</button>
          </div>
        )}
        {step === 'error' && (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            <div style={{ fontSize: '52px', marginBottom: '16px' }}>⚠️</div>
            <h3 style={{ color: '#ef4444', margin: '0 0 12px', fontFamily: 'Georgia, serif', fontWeight: 'normal' }}>Error Sending Documents</h3>
            <p style={{ color: '#8a9ab8', fontSize: '13px', lineHeight: 1.7, margin: '0 auto 28px', maxWidth: '360px' }}>{errorMsg}</p>
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
              <button onClick={() => setStep('confirm')} style={{ background: 'rgba(255,255,255,0.08)', color: '#c4cdd8', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '2px', padding: '10px 24px', cursor: 'pointer', fontSize: '12px' }}>Try Again</button>
              <button onClick={onClose} style={{ background: 'transparent', color: '#6b7280', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '2px', padding: '10px 24px', cursor: 'pointer', fontSize: '12px' }}>Close</button>
            </div>
          </div>
        )}
      </div>
...
function AccountProfile({ portalUser }) {
  ...
  const save = async () => {
    ...
    try {
      await updateUser(portalUser.username || portalUser.email, form);
  ...
  return (
    <div style={{ maxWidth: '520px' }}>
      ...
      {[{ key: 'name', label: 'Full Name' }, { key: 'email', label: 'Email Address', type: 'email' }, { key: 'phone', label: 'Phone Number', placeholder: '(216) 555-0123' }, { key: 'address', label: 'Mailing Address', placeholder: '123 Main St, Cleveland, OH 44101' }].map(({ key, label, type = 'text', placeholder = '' }) => (
...
function AccountDocuments({ portalUser }) {
  ...
  const loadDocs = async () => {
    try {
      const reqs = await SignNowRequestDB.listForEmail(portalUser.email);
      setRequests(reqs);
  ...
  const downloadSigned = async (req, doc) => {
    const key = `${req.id}-${doc.documentId}`;
  ...
  const allDocs = requests.flatMap(req => {
    let docs = []; try { docs = JSON.parse(req.documents || '[]'); } catch {}
    return docs.map(d => ({ ...d, reqId: req.id, req }));
  });
  ...
  if (allDocs.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: '48px 20px' }}>
        ...
        <p style={{ color: '#374151', fontSize: '13px', maxWidth: '340px', margin: '0 auto' }}>Click <strong style={{ color: GOLD }}>"Request Investment Documents"</strong> at the top of the portal to initiate the signature process.</p>
      </div>
    );
  }
      ...
      {outstanding.length > 0 && (
        <div style={{ marginBottom: '32px' }}>
          ...
          {outstanding.map((doc, i) => {
            const s2 = sc[doc.status] || sc.pending;
            return (
              <div key={i} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '2px', padding: '18px 20px', marginBottom: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px' }}>
                <div>
                  <div style={{ color: '#e8e0d0', fontWeight: 'bold', fontSize: '14px', marginBottom: '4px' }}>📄 {doc.name}</div>
                  <div style={{ color: '#4a5568', fontSize: '11px' }}>Sent {doc.req?.sentAt ? new Date(doc.req.sentAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—'}</div>
                </div>
        ...
        <div>
          ...
          {completed.map((doc, i) => (
            <div key={i} style={{ background: 'rgba(74,222,128,0.04)', border: '1px solid rgba(74,222,128,0.15)', borderRadius: '2px', padding: '18px 20px', marginBottom: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px' }}>
              <div>
                <div style={{ color: '#e8e0d0', fontWeight: 'bold', fontSize: '14px', marginBottom: '4px' }}>✅ {doc.name}</div>
                <div style={{ color: '#4a5568', fontSize: '11px' }}>Signed {doc.completedAt ? new Date(doc.completedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—'}</div>
              </div>
              {doc.documentId && (
                <button onClick={() => downloadSigned(doc.req, doc)} disabled={downloadingId === `${doc.reqId}-${doc.documentId}`} style={{ background: 'rgba(74,222,128,0.12)', color: '#4ade80', border: '1px solid rgba(74,222,128,0.3)', borderRadius: '2px', padding: '7px 16px', cursor: 'pointer', fontSize: '11px', flexShrink: 0 }}>
                  {downloadingId === `${doc.reqId}-${doc.documentId}` ? 'Downloading…' : '↓ Download PDF'}
                </button>
              )}
            </div>
          ))}
...
function AccountTab({ portalUser }) {
  ...
  return (
    <div id="portal-tab-content">
      ...
      <p style={{ color: '#4a5568', fontSize: '12px', margin: '0 0 24px' }}>View your investment details, documents, and update your contact information.</p>
      <div style={{ display: 'flex', gap: '0', borderBottom: '1px solid rgba(255,255,255,0.08)', marginBottom: '28px' }}>
        {[['investment', '📊 Investment'], ['documents', '📄 Documents'], ['profile', '👤 Profile']].map(([id, label]) => (
...
async function downloadFile(url, filename) { const res = await fetch(url); const blob = await res.blob(); const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = filename; a.click(); URL.revokeObjectURL(u); }
const PPM_INDEX = [
  { id: 'cover', label: 'Cover Page', page: 1 }, { id: 'notices', label: 'Notices', page: 2 },
  { id: 'summary', label: 'Summary of Terms', page: 4 }, { id: 'focus', label: 'Focus of the Offering', page: 5 },
  { id: 'revenue', label: 'Revenue Projections & Milestones', page: 6 }, { id: 'subscribe', label: 'How to Subscribe', page: 7 },
  { id: 'intro', label: 'Rosie AI Introduction', page: 9 }, { id: 'positioning', label: 'Unique Positioning', page: 10 },
  { id: 'leadership', label: 'Leadership & Architects', page: 12 }, { id: 'orgchart', label: 'Organizational Chart', page: 14 },
  { id: 'capitalization', label: 'Capitalization & Management', page: 15 }, { id: 'fiduciary', label: 'Fiduciary Responsibilities', page: 16 },
  { id: 'risk-mgmt', label: 'Risk Management & Exit Strategy', page: 18 }, { id: 'terms', label: 'Terms of the Offering', page: 19 },
  { id: 'subscribing', label: 'Subscribing to the Offering', page: 20 }, { id: 'proceeds', label: 'Use of Investor Proceeds', page: 22 },
  { id: 'rights', label: 'Rights & Liabilities', page: 25 }, { id: 'alloc', label: 'Allocation & Distributions', page: 26 },
  { id: 'sub-proc', label: 'Subscription Procedures', page: 28 }, { id: 'risk-factors', label: 'Risk Factors', page: 42 },
  { id: 'erisa', label: 'ERISA Considerations', page: 50 }, { id: 'state-notices', label: 'State-Specific Legal Notices', page: 44 },
    ...
    <div style={{ display: 'flex', gap: '0', minHeight: '700px' }}>
      <div style={{ width: '220px', flexShrink: 0, borderRight: '1px solid rgba(255,255,255,0.08)', overflowY: 'auto', maxHeight: '80vh' }}>
        ...
        <div style={{ padding: '16px', borderTop: '1px solid rgba(255,255,255,0.07)', marginTop: '8px' }}>
          <button onClick={() => { analytics.trackDownload('RosieAI_PPM.pdf', 'pdf'); downloadFile(PPM_PDF_URL, 'RosieAI_PPM.pdf'); }} style={{ width: '100%', background: 'rgba(184,147,58,0.15)', color: GOLD, border: `1px solid rgba(184,147,58,0.3)`, borderRadius: '2px', padding: '10px', cursor: 'pointer', fontSize: '12px' }}>↓ Download PPM</button>
        </div>
      </div>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 20px', borderBottom: '1px solid rgba(255,255,255,0.07)', background: 'rgba(0,0,0,0.2)', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <button onClick={() => { analytics.trackDownload('RosieAI_PPM.pdf', 'pdf'); downloadFile(PPM_PDF_URL, 'RosieAI_PPM.pdf'); }} style={{ background: 'rgba(184,147,58,0.15)', color: GOLD, border: `1px solid rgba(184,147,58,0.3)`, borderRadius: '2px', padding: '6px 14px', cursor: 'pointer', fontSize: '11px', marginLeft: '4px' }}>↓ Download</button>
          </div>
        </div>
...
function SubscriptionAgreements({ onRequestDocuments }) {
  ...
  const handleDownload = (doc) => { analytics.trackDownload(doc.name + '.pdf', 'pdf'); downloadFile(doc.url, doc.name + '.pdf'); };
  return (
    <div id="portal-tab-content">
      <div style={{ display: 'flex', gap: '0', minHeight: '600px' }}>
        <div style={{ width: '220px', flexShrink: 0, borderRight: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          <div style={{ padding: '16px', marginTop: '16px', borderTop: '1px solid rgba(255,255,255,0.07)' }}>
            {PDF_DOCS.map(doc => (
              <button key={doc.id} onClick={() => handleDownload(doc)} style={{ display: 'block', width: '100%', textAlign: 'left', background: 'transparent', border: 'none', color: '#8a9ab8', padding: '6px 0', cursor: 'pointer', fontSize: '12px' }}>↓ {doc.name}</button>
            ))}
          </div>
        </div>
        <div style={{ flex: 1 }}>
...
function InvestorUpdates({ isAdmin }) {
  ...
  async function loadUpdates() {
    ...
    try {
      ...
      if (arr.length === 0) {
        for (const s of [
          { title: 'Q1 2025 Performance Update', content: 'Revenue grew 47% QoQ to $95K MRR. We onboarded 12 new enterprise clients in solar and insurance verticals.', category: 'Financial Update', author: 'Management Team', publishedAt: '2025-04-01T00:00:00.000Z' },
          { title: 'Product Launch: Rosie 2.0', content: 'Today we launched Rosie 2.0 with a new real-time conversation AI engine with less than 150ms latency.', category: 'Product Update', author: 'Product Team', publishedAt: '2025-02-28T00:00:00.000Z' },
          ...
          <div key={update.id} style={{ display: 'flex', gap: '20px', paddingBottom: '32px' }}>
            ...
            <div style={{ flex: 1, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '2px', padding: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px', flexWrap: 'wrap', gap: '8px' }}>
                <div>
                  <span style={{ display: 'inline-block', padding: '3px 10px', borderRadius: '2px', background: `${catColors[update.category]}22`, color: catColors[update.category] || '#8a9ab8', fontSize: '10px', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '8px' }}>{update.category}</span>
                  <h3 style={{ color: '#e8e0d0', margin: '0', fontSize: '16px', fontFamily: 'Georgia, serif', fontWeight: 'normal' }}>{update.title}</h3>
                </div>
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{ color: GOLD, fontSize: '13px' }}>{new Date(update.publishedAt || update.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</div>
...
function RaiseProgress() {
  ...
  const INVESTED_T = Number(s.investedTarget) || 500000;
      ...
      {[{ label: 'Committed Capital', pct: Math.min((COMMITTED / TOTAL) * 100, 100), color: GOLD, sub: `${fmt(COMMITTED)} of ${fmt(TOTAL)} raise` }, { label: 'Invested Capital', pct: Math.min((INVESTED / INVESTED_T) * 100, 100), color: '#4ade80', sub: `${fmt(INVESTED)} of ${fmt(INVESTED_T)} deployed` }].map(({ label, pct, color, sub }) => (
        <div key={label} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '2px', padding: '20px' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: '2px', height: '6px', marginBottom: '10px', overflow: 'hidden' }}>
            <div style={{ background: `linear-gradient(90deg, ${color}88, ${color})`, width: `${pct}%`, height: '100%', borderRadius: '2px', transition: 'width 1s ease' }} />
          </div>
...
function PortalHome({ setActiveTab, portalUser, onRequestDocuments }) {
  ...
  const navCards = [
    { tab: 'account', icon: '👤', title: 'My Account', desc: 'View your investment summary, signed documents, and update your contact info.' },
    ...
    <div style={{ minHeight: '100vh', background: DARKER, fontFamily: 'Georgia, serif', color: '#e8e0d0' }}>
      ...
      <nav style={{ background: DARK, borderBottom: '1px solid rgba(184,147,58,0.2)', padding: '0 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '64px', position: 'sticky', top: 0, zIndex: 200 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'nowrap' }}>
          <button onClick={() => setShowRequestDocs(true)} style={{ background: 'linear-gradient(135deg,#b8933a,#d4aa50)', color: DARK, border: 'none', borderRadius: '2px', padding: '9px 16px', cursor: 'pointer', fontWeight: '700', fontSize: '11px', letterSpacing: '1.5px', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
            ✍️ Request Investment Documents
          </button>
