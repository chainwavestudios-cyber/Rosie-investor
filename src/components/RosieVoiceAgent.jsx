...
export default function RosieVoiceAgent({ userName = 'Steph', investorId = null, investorEmail = null, inline = false }) {
  ...
  const chatBody = (
    <>
      <div style={{ flex:1, overflowY:'auto', padding:'16px', minHeight: inline ? '125px' : undefined }}>
        {transcript.length === 0 && (
  ...
  if (inline) {
    return (
      <div style={{ display:'flex', flexDirection:'column', background:'rgba(0,0,0,0.2)', minHeight:'200px' }}>
        {chatBody}
      </div>
    );
  }
