...
export default function LiveAssistant({ isCallActive = false, lead = null, currentLead = null }) {
  ...
  const checkCoach = async (allT) => {
    const now = Date.now();
    if (now - lastCoachRef.current < 15000) return;
        ...
        <div onMouseDown={onMouseDown} style={{ padding:'10px 14px', background:'rgba(184,147,58,0.1)', borderBottom:'1px solid rgba(184,147,58,0.2)', cursor:'grab', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          ...
          <div style={{ display:'flex', gap:'4px', alignItems:'center' }}>
            <button onClick={() => { setShowResearch(r => !r); if (!research && !researchLoading) runResearch(); }}
              title="Research lead"
              style={{ background: showResearch ? 'rgba(96,165,250,0.2)' : 'rgba(255,255,255,0.05)', border:`1px solid ${showResearch ? 'rgba(96,165,250,0.4)' : 'rgba(255,255,255,0.1)'}`, color: showResearch ? '#60a5fa' : '#6b7280', borderRadius:'4px', padding:'2px 7px', cursor:'pointer', fontSize:'11px' }}>🔍</button>
            <button onClick={() => setCoachMode(c => !c)}
              title="Coach mode"
              style={{ background: coachMode ? 'rgba(167,139,250,0.2)' : 'rgba(255,255,255,0.05)', border:`1px solid ${coachMode ? 'rgba(167,139,250,0.4)' : 'rgba(255,255,255,0.1)'}`, color: coachMode ? '#a78bfa' : '#6b7280', borderRadius:'4px', padding:'2px 7px', cursor:'pointer', fontSize:'11px' }}>🎯</button>
