...
function generateHTMLReport({ lead, transcript, qaLog, coachTips, intentResult, callStartTime, callEndTime, kbName, selectedKbName }) {
  ...
  const prospectPct = Math.round((s0words / totalWords) * 100); // speaker0 = ch0 = remote = prospect
  const agentPct = 100 - prospectPct;                            // speaker1 = ch1 = local  = agent
...
${transcript.map(t=>`<div class="t-line"><span class="t-spk ${t.speaker===0||t.speaker===null||t.speaker===undefined?'t-spk-0':'t-spk-1'}">${t.speaker===1?'🎙 Agent':'👤 Prospect'}</span><span class="t-txt">${t.text}</span><span class="t-time">${new Date(t.time).toLocaleTimeString()}</span></div>`).join('')}
</div>
