    ...
    try {
      ...
      ws.onmessage = e => {
        try {
          ...
          // Capture sentiment and speaker from Deepgram
          const sentiment  = alt?.sentiment  || null;   // { sentiment: 'positive'|'negative'|'neutral', sentiment_score: 0-1 }
          const speaker    = data?.channel?.alternatives?.[0]?.words?.[0]?.speaker ?? null; // 0 or 1
          ...
          const entry = {
            text,
            time:      new Date(),
            speaker,                              // 0 = likely agent, 1 = likely prospect
            sentiment: sentLabel || sentiment,    // positive / negative / neutral
            sentScore: sentScore ?? null,
          };
