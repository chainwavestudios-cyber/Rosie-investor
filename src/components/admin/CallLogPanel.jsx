...
// Known Twilio lines — label them numerically
const KNOWN_LINES = [
  { key: 'line0', label: 'Phone 0' },
  { key: 'line1', label: 'Phone 1' },
  { key: 'line2', label: 'Phone 2' },
];
...
// ── Live Line Status Bar ────────────────────────────────────────────────────
function LiveLineBar({ lineNum, logs }) {
  const [elapsed, setElapsed] = useState(0);

  // Find the most recent answered/in-progress log for this line index
  // We identify lines by to/fromNumber order in the log set
  const allNumbers = [...new Set(logs.flatMap(l => [l.fromNumber, l.toNumber]).filter(Boolean))];
  const ourNumber = allNumbers[lineNum] || null;

  const activeLog = logs.find(l =>
    (l.fromNumber === ourNumber || l.toNumber === ourNumber) &&
  ...
  const displayName = activeLog?.callerName
    || (activeLog?.direction === 'inbound' ? activeLog?.fromNumber : activeLog?.toNumber)
    || '';

  const displayPhone = activeLog?.direction === 'inbound'
    ? activeLog?.fromNumber
    : activeLog?.toNumber;

  const barColor = isActive ? '#4ade80' : '#ef4444';
  const barBg    = isActive ? 'rgba(74,222,128,0.08)' : 'rgba(239,68,68,0.08)';
  const barBorder= isActive ? 'rgba(74,222,128,0.3)'  : 'rgba(239,68,68,0.25)';
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        flexShrink: 0,
        animation: isActive ? 'linePulse 1.2s ease-in-out infinite' : 'none',
      }} />

      {/* Line label */}
      <div style={{ color: barColor, fontSize: '11px', fontWeight: 'bold', letterSpacing: '0.5px', flexShrink: 0 }}>
        Phone {lineNum}
      </div>
      ...
      {isActive ? (
        <div style={{ flex: 1, display: 'flex', gap: '8px', alignItems: 'center', minWidth: 0 }}>
          <span style={{ color: '#e8e0d0', fontSize: '11px', fontWeight: 'bold', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {displayName || 'Active Call'}
          </span>
          {displayPhone && (
            <span style={{ color: '#6b7280', fontSize: '10px', fontFamily: 'monospace', flexShrink: 0 }}>{displayPhone}</span>
          )}
          {/* Rising timer */}
          <span style={{
            marginLeft: 'auto', color: '#4ade80', fontSize: '13px',
        ...
        <div style={{ flex: 1, color: '#4a5568', fontSize: '11px' }}>Disconnected</div>
      )}
    </div>
  );
}
...
export default function CallLogPanel({ onClose, onOpenLead }) {
  ...
  useEffect(() => {
    loadLogs();
    // Poll for active calls every 5s
  ...
  // Build unique numbers for filter, ordered by first appearance
  const allNumbers = [...new Set(logs.flatMap(l => [l.toNumber, l.fromNumber]).filter(Boolean))];
  ...
  return (
    <>
      ...
      <div style={{
        ...
        width: '500px', maxHeight: '85vh',
        background: '#080f1c', border: `1px solid rgba(184,147,58,0.3)`,
        borderRadius: '10px', boxShadow: '0 20px 60px rgba(0,0,0,0.85)',
        ...
        {/* ── Live Line Status Bars ── */}
        <div style={{ padding: '10px 14px', borderBottom: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
          <div style={{ color: '#4a5568', fontSize: '9px', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '7px' }}>Live Line Status</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
            {[0, 1, 2].map(n => (
              <LiveLineBar key={n} lineNum={n} logs={logs} />
            ))}
          </div>
        </div>
        ...
        <div style={{ padding: '10px 18px', borderBottom: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
          ...
          >
            <option value="all" style={{ background: '#0a1220', color: '#e8e0d0' }}>
              📱 Phone #s — All ({logs.length} calls)
            </option>
            {allNumbers.map((num, i) => (
              <option key={num} value={num} style={{ background: '#0a1220', color: '#e8e0d0' }}>
                Phone {i} — {num} ({logs.filter(l => l.fromNumber === num || l.toNumber === num).length} calls)
              </option>
            ))}
          </select>
        </div>
