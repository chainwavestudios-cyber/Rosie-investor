...
export default function KnowledgeBaseManager({ IntentEngineTuner, CoachRulesTuner }) {
  ...
  const startEdit = (e) => { setEditingId(e.id); setEditQ(e.question||''); setEditA(e.answer||''); setEditCat(e.category||'faq'); setEditTags(e.tags||''); };

  const saveEdit = async () => {
    if (!editQ.trim() || !editA.trim()) return;
    setEditSaving(true);
    try { await base44.entities.KnowledgeBase.update(editingId, { question:editQ.trim(), answer:editA.trim(), category:editCat, tags:editTags.trim() }); setEditingId(null); await load(); } catch (e) { alert('Save failed: ' + e.message); }
    setEditSaving(false);
  };

  const load = async () => {
    setLoading(true);
    try { const all = await base44.entities.KnowledgeBase.list('-created_date', 1000); setEntries(all || []); } catch {}
    setLoading(false);
  };
  useEffect(() => { load(); }, []);
  ...
  const addManual = async () => {
    ...
    try { await base44.entities.KnowledgeBase.create({ question:q.trim(), answer:a.trim(), category:cat, tags:tags.trim(), source:'manual' }); setQ(''); setA(''); setTags(''); setSaveMsg('✓ Entry added'); await load(); } catch (e) { setSaveMsg('Error: '+e.message); }
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      let saved = 0;
      for (const entry of extracted) { try { await base44.entities.KnowledgeBase.create({ question:entry.question, answer:entry.answer, category:entry.category||'faq', source:file.name, tags:entry.tags||'' }); saved++; } catch {} }
      setUploadMsg(`✓ Extracted and saved ${saved} entries from "${file.name}"`); await load();
    } catch (e) { setUploadMsg('Error: '+e.message); }
    setUploading(false); setUploadProgress(''); setTimeout(() => setUploadMsg(''), 6000);
  };

  const handleScrape = async () => {
    if (!scrapeUrl.trim()) return; setScraping(true); setScrapeMsg('Fetching and analyzing page…');
    try {
      const result = await base44.functions.invoke('kbScrapeUrl', { url:scrapeUrl.trim() });
      const extracted = result?.data?.entries || []; let saved=0;
      for (const entry of extracted) { try { await base44.entities.KnowledgeBase.create({ question:entry.question, answer:entry.answer, category:entry.category||'faq', source:scrapeUrl.trim(), tags:entry.tags||'' }); saved++; } catch {} }
      setScrapeMsg(`✓ Scraped and saved ${saved} entries from ${scrapeUrl}`); setScrapeUrl(''); await load();
    } catch (e) { setScrapeMsg('Error: '+e.message); }
    setScraping(false); setTimeout(() => setScrapeMsg(''), 6000);
  };

  const deleteEntry = async (id) => { setDeleting(id); try { await base44.entities.KnowledgeBase.delete(id); await load(); } catch {} setDeleting(null); };
  ...
  const filtered = entries.filter(e => filterCat==='all'||e.category===filterCat).filter(e => !search||`${e.question} ${e.answer} ${e.tags||''}`.toLowerCase().includes(search.toLowerCase()));
    ...
    <div style={{ fontFamily:'Georgia, serif' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'24px' }}>
        <div>
          ...
          <p style={{ color:'#6b7280', fontSize:'13px', margin:0 }}>{entries.filter(e=>e.category!=='raw_document').length} entries · {sources.length} sources · Used by <strong style={{ color:GOLD }}>Rosie AI</strong> and the <strong style={{ color:'#a78bfa' }}>Live Call Assistant</strong></p>
        </div>
      ...
      {section === 'entries' && (
        <div>
          ...
          {!loading && filtered.length === 0 && <div style={{ textAlign:'center', padding:'60px', color:'#4a5568' }}><div style={{ fontSize:'48px', marginBottom:'12px' }}>🧠</div><p>No entries yet. Upload a document, scrape a URL, or add Q&A manually.</p></div>}
                ...
                {editingId === e.id ? (
                  <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
                    ...
                    <div style={{ display:'flex', gap:'8px', alignItems:'center' }}>
                      <select value={editCat} onChange={ev=>setEditCat(ev.target.value)} style={{ ...inp2, width:'140px', padding:'6px 10px', fontSize:'12px', cursor:'pointer' }}>{CATEGORIES.filter(c=>c!=='all').map(c=><option key={c} value={c}>{c}</option>)}</select>
                      ...
                      <button onClick={saveEdit} disabled={editSaving} style={{ background:'linear-gradient(135deg,#b8933a,#d4aa50)', color:'#0a0f1e', border:'none', borderRadius:'4px', padding:'6px 16px', cursor:'pointer', fontSize:'12px', fontWeight:'bold', whiteSpace:'nowrap' }}>{editSaving?'⏳':'✓ Save'}</button>
      ...
      {section === 'upload' && (
        <div style={{ maxWidth:'640px' }}>
          ...
          <p style={{ color:'#6b7280', fontSize:'13px', margin:'0 0 24px', lineHeight:1.7 }}>Upload a PDF, Word doc, or text file. The AI will read the entire document and extract every useful Q&A pair automatically. A 56-page document produces 60-100+ entries.</p>
          <div onClick={() => !uploading&&fileRef.current?.click()} style={{ border:`2px dashed ${uploading?'rgba(184,147,58,0.5)':'rgba(255,255,255,0.15)'}`, borderRadius:'8px', padding:'48px', textAlign:'center', cursor:uploading?'default':'pointer', background:'rgba(255,255,255,0.02)', transition:'all 0.2s' }} onMouseEnter={e=>{ if(!uploading)e.currentTarget.style.borderColor=GOLD; e.currentTarget.style.background='rgba(184,147,58,0.04)'; }} onMouseLeave={e=>{ e.currentTarget.style.borderColor='rgba(255,255,255,0.15)'; e.currentTarget.style.background='rgba(255,255,255,0.02)'; }}>
              ...
              {sources.map(src => {
                ...
                return (
                  <div key={src} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 12px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'4px', marginBottom:'4px' }}>
                    ...
                    <button onClick={async()=>{ if(!window.confirm(`Delete all entries from "${src}"?`))return; const toDelete=entries.filter(e=>e.source===src); for(const e of toDelete){try{await base44.entities.KnowledgeBase.delete(e.id);}catch{}} await load(); }} style={{ background:'none', border:'none', color:'#ef444466', cursor:'pointer', fontSize:'13px' }}>× Remove</button>
                  </div>
                );
              })}
            </div>
      ...
      {section === 'scrape' && (
        <div style={{ maxWidth:'640px' }}>
          ...
          <p style={{ color:'#6b7280', fontSize:'13px', margin:'0 0 24px', lineHeight:1.7 }}>Enter a URL and the AI will fetch the page, strip the noise, and extract every useful Q&A pair. Works best on product pages, FAQs, about pages, and documentation.</p>
