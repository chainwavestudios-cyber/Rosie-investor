...
export default function GlobalCalendar({ users = [], setContactCard, setView, setOpenLeadId }) {
  ...
  const invEvents = allAppts.map(a => ({ id:a.id, type:'investor', raw:a, title:a.title||'Appointment', name:a.investorName||'', dateTime:new Date(a.scheduledAt), status:a.status||'scheduled', notes:a.notes||'', phone:null, color:'#60a5fa' }));
  const leadEvents = allLeads.filter(l => l.callbackAt && l.status !== 'callback_later' && l.status !== 'converted' && l.status !== 'not_interested').map(l => ({ id:l.id, type:'lead', raw:l, title:'Lead Callback', name:`${l.firstName} ${l.lastName}`, dateTime:new Date(l.callbackAt), status:'scheduled', notes:'', phone:l.phone, color:'#a78bfa' }));
                    ...
                    <div key={`${evt.type}-${evt.id}`}
                      ...
                      onClick={() => {
                        if (evt.type==='investor') {
                          ...
                          if (u) { setContactCard(u); } else { base44.entities.InvestorUser.filter({id:investorUserId}).then(rows => { if (rows?.[0]) setContactCard(rows[0]); }).catch(()=>{}); }
                        } else if (evt.type==='lead') { setOpenLeadId(evt.id); setView('leads'); }
                      }}
